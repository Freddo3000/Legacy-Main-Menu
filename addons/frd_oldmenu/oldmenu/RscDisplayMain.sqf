...
switch _mode do {
	...
	case "onLoad": {
		...
		//{		
			...
			// {
				// (_display displayctrl 1896) ctrlEnable false;	//Options
		...
		if(!isAutotest) then
		{
			if ((configName (configFile >> "CfgPatches" >> "A3_Missions_F_EPC")) != "") then 
			{
				...
				{
					//Showing the Bootcamp Welcome Screen once per profile
					if ((profileNamespace getVariable ["BIS_welcomeScreenBootcampShown", 0]) <= 0) then 
					{
						//Test Bootcamp is installed
						if ((configName (configFile >> "CfgPatches" >> "A3_Missions_F_Bootcamp")) != "") then 
						{				
							//If any welcome was not already shown to any profile this session, show it.
							if (!(uiNamespace getVariable ["BIS_welcomeScreenBootcampShown", false])) then 
							{
								_display createDisplay "RscDisplayWelcome";
							} 
							else 
	...
	*/
	...
	case "playButton": {
		...
		if(uiNamespace getvariable "BIS_MainMenu_isPlayExpanded") then
		{
			...
			_control ctrlSetPosition [(6 * GUI_GRID_W + GUI_GRID_X), (14.2 * GUI_GRID_H + GUI_GRID_Y)];//13.1 //_offset
			_control ctrlCommit _upperPartTime;
		...
		else
		{
			...
			//Player's name
			_control = _display displayctrl 109;
			_control ctrlSetPosition [(6 * GUI_GRID_W + GUI_GRID_X), ((9.8 + _offsetCampaign) * GUI_GRID_H + GUI_GRID_Y)];//9.8 //_offset // + _offsetShowcases
			_control ctrlCommit _upperPartTime;
	...
	case "learnButton": {
		...
		else
		{
			...
			//Player's name
			_control = _display displayctrl 109;
			_control ctrlSetPosition [(6 * GUI_GRID_W + GUI_GRID_X), (8.7 * GUI_GRID_H + GUI_GRID_Y)];//6.5 //_offset
			_control ctrlCommit _upperPartTime;
	...
	case "optionsButton": {
		...
		if(uiNamespace getvariable "BIS_MainMenu_isOptionsExpanded") then
		{
			...
			//Options button
			_control = _display displayctrl 1896;
		...
		else
		{
			...
			//Player's name
			_control = _display displayctrl 109;
			_control ctrlSetPosition [(6 * GUI_GRID_W + GUI_GRID_X), (7.6 * GUI_GRID_H + GUI_GRID_Y)];//6.5 //_offset
			_control ctrlCommit _upperPartTime;
		...
		// {
			...
			// {
				...
				// {
					...
					// if(uiNamespace getvariable "BIS_MainMenu_isOptionsExpanded") then
					// {	
						// ['optionsButton', [_display displayctrl 1896], ''] call RscDisplayMain_script; //simulate click on Options button (collapse)
					// };
				...
				// {
					// //enable buttons
					// (_display displayctrl 105) ctrlEnable true;	//Multiplayer
					// (_display displayctrl 1896) ctrlEnable true;	//Options
